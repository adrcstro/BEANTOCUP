...
if ($result) {
    ...
    echo '<div class="modal fade" id="MenuDetailsModal" tabindex="-1" role="dialog" aria-labelledby="MenuDetailsModalLabel" aria-hidden="true">';
    echo '<div class="modal-dialog" role="document">';
    echo '<div class="modal-content">';
    echo '<div class="modal-header">';
    echo '<h5 class="modal-title" id="MenuDetailsModalLabel">Menu Details</h5>';
    echo '<button type="button" class="close" data-dismiss="modal" aria-label="Close">';
    echo '<span aria-hidden="true">&times;</span>';
    echo '</button>';
    echo '</div>';
    echo '<div class="modal-body">';
    echo '<p id="menuName"></p>';
    echo '<p id="menuPrice"></p>';
    echo '<p id="menuBody"></p>';
    echo '<img id="menuImage" class="img-fluid" alt="Menu Image">';
    echo '</div>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $trimmed_body = strlen($row['Body']) > 10 ? substr($row['Body'], 0, 10) . '...' : $row['Body'];
        echo '<p class="card-text">' . $trimmed_body . ' <button data-toggle="modal" data-target="#MenuDetailsModal" class="read-more" onclick="openMenuDetailsModal(' . $row['MenuID'] . ')">Read More</button></p>';
        echo '<div class="card-buttons">';
        echo '<button class="btn btn btn-sm-custom mr-1"><i class="fas fa-shopping-cart"></i></button>';
        echo '<button data-toggle="modal" data-target="#MenuDetailsModal" class="btn btn btn-sm-custom mr-1" onclick="openMenuDetailsModal(' . $row['MenuID'] . ')"><i class="fas fa-shopping-bag"></i></button>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
...
// JavaScript for updating modal content dynamically
echo '<script>';
echo 'function openMenuDetailsModal(menuID) {';
echo '  var modal = $("#MenuDetailsModal");';
echo '  var name = $("#menuName");';
echo '  var price = $("#menuPrice");';
echo '  var body = $("#menuBody");';
echo '  var image = $("#menuImage");';
echo '  $.ajax({';
echo '    type: "POST",';
echo '    url: "get_menu_details.php",'; // Create a separate PHP file to handle AJAX requests
echo '    data: { menuID: menuID },';
echo '    success: function(response) {';
echo '      var data = JSON.parse(response);';
echo '      name.text("Name: " + data.name);';
echo '      price.text("Price: " + data.price);';
echo '      body.text("Description: " + data.body);';
echo '      image.attr("src", "uploads/" + data.image);';
echo '    }';
echo '  });';
echo '}';
echo '</script>';
...
<div class="modal fade" id="BuyNow">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <button  style="background-color: #E48F45; color: #fff;" data-dismiss="modal" data-toggle="modal"  data-target="#"   type="button" class="btn  btn-sm m-1">
            <i class="bi bi-bag-fill"></i> Buy Now</button>
            </div>
