...
if (mysqli_num_rows($result) > 0) {
    ...
    echo "<p>Customer ID: {$userData['CustomerID']}</p>";
    echo "<p>Email: {$userData['Email']}</p>";
    echo "<p>Phone: {$userData['Phone']}</p>";
    echo "<p>Home Address: {$userData['HomeAddress']}</p>";
    echo '</div>';
    
    // You can replace the following image URL with the actual URL of the image you want to display.
    $imageUrl = 'Images/order.svg';
    echo '<div style="flex: 1; text-align: right;">';
    echo "<img src=\"$imageUrl\" alt=\"User Image\" style=\"max-width: 100%; height: auto;\">";
    echo '</div>';
    echo '</div>';
