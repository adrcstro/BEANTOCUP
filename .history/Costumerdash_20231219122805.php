...
// Check if the user is already being processed in this session
if (isset($_SESSION['processing_user']) && $_SESSION['processing_user']['session_id'] !== $sessionID) {
    // Another session is already processing the user, wait or handle accordingly
    echo "This account is currently being processed by another user. Please wait or try again later.";
    exit();
}

// Set the current user as the processing user with the session ID
$_SESSION['processing_user'] = array('username' => $loggedInUsername, 'session_id' => $sessionID);
...
if (mysqli_num_rows($result) > 0) {
    ...
    // Rest of your Costumerdash.php code goes here
} else {
    // No matching user found
    echo "Error fetching user details.";
}

// Release the lock for the current user
