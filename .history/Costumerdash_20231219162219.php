...
// Check if user-specific data is already stored in the session
if (!isset($_SESSION['CostumerID']) || (time() > $_SESSION['timeout'])) {
    // Expire the session and redirect to login if not logged in or session timed out
    session_unset();
    session_destroy();
    header('Location: login.php');
    exit();
...
// Check if user-specific data is already stored in the session
if (!isset($_SESSION['user_data'][$sessionIdentifier][$loggedInUsername])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $_SESSION['user_data'][$sessionIdentifier][$loggedInUsername] = $userData;
    } else {
...
// Update last activity time on every page request
