...
<script>
  ...
  function insertText() {
    ...
    // Set the initial price to the current value of the total input
    var initialPrice = parseFloat(inputElement.value);
    // Call the updateTotal function to ensure the initial total is set correctly
    updateTotal(initialPrice);
  }

  // Additional functionality for quantity and total
  $(document).ready(function() {
    // Function to update the total based on quantity
    function updateTotal(initialPrice) {
      var quantity = parseInt($('#quantity').val());
      var total = initialPrice * quantity;
      $('#total').val(total.toFixed(2)); // Ensure the total is formatted to two decimal places
    }
    ...
    $('#increase-btn').on('click', function() {
      ...
      updateTotal(parseFloat($('#total').val()));
    });
