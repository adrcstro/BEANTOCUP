...
<!-- Add an ID to the total span where the total price will be displayed -->
<div id="total" class="input-group mb-3 d-flex justify-content-center align-items-center" style="width: 500px;">
    <span class="input-group-text">Total:</span>
    <input type="text" class="form-control" id="totalValue" aria-label="Recipient's username" aria-describedby="button-addon2" readonly>
    <button style="background-color: #E48F45; color: #fff;" class="btn btn-outline-secondary custom-width-btn" type="button" id="button-addon2">
        <i class="bi bi-currency-dollar"></i>Amount
    </button>
</div>

<!-- Modify the quantity input to have an ID -->
<input type="text" class="form-control text-center" id="quantity" value="1" readonly>

<script>
    // Function to handle quantity change events
    function updateTotal() {
        var quantity = parseInt(document.getElementById("quantity").value);
        var pricePerItem = parseFloat(document.getElementById("total").value); // Use the appropriate ID for the price input

        // Calculate the total price
        var totalPrice = quantity * pricePerItem;

        // Update the total value
        document.getElementById("totalValue").value = totalPrice.toFixed(2);
    }

    // Attach the updateTotal function to quantity change events
