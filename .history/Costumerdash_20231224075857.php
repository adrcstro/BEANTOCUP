...
<script>
  $(document).ready(function () {
    ...
    var itemPrice; // Variable to store the item price

    // Function to update the total based on the current quantity
    function updateTotal() {
      var currentValue = parseInt(quantityInput.val());
      if (!isNaN(itemPrice) && !isNaN(currentValue)) {
        totalInput.val((currentValue * itemPrice).toFixed(2));
      } else {
        totalInput.val('0.00');
      }
    }

    // Replace '123' with the actual menu ID you want to fetch the price for
    var menuID = 'MenuID';

    // Fetch the item price when the page loads
    $.ajax({
      type: 'POST',
      ...
      success: function (data) {
        ...
        if (data && data.Price !== undefined) {
          ...
          // Set the initial total based on the default quantity
          updateTotal();
        } else {
      ...
      error: function (xhr, status, error) {
        console.error('Error fetching data from the server:', xhr.responseText);
      }
    });
    ...
    $('#increase-btn').click(function () {
      ...
      // Update the total based on the current quantity
      updateTotal();
    });
