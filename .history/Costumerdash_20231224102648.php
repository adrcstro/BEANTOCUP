...
document.addEventListener("DOMContentLoaded", function () {
  ...
  $(document).on('click', '.buy-now-btn', function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        // Update the quantity to 1 when a new item is selected
        $('#quantity').val(1);

        // Enable the Increase button when a new item is selected
        $('#increase-btn').prop('disabled', false);

        // Update the total when a new item is selected
        updateTotal();
      },
  ...
  function updateTotal() {
    ...
    var pricePerItem = parseFloat($('#total').val());
