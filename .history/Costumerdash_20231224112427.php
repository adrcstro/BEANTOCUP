...
<script>
    ...
    <input type="text" class="form-control" id="quantity" aria-label="Quantity" aria-describedby="button-addon1" value="1">
...
document.addEventListener("DOMContentLoaded", function() {
    var menuID = 0; // Initialize menuID, or set it to an appropriate default value
            ...
            success: function(data) {
                if (data && typeof data.Price !== 'undefined') {
                    ...
                    if (!isNaN(price) && isFinite(price)) {
                        ...
                        $('#total').val('Error: Invalid Price');
                    }
                } else {
                    console.error('Invalid data received from server');
                    $('#total').val('Error: Invalid Data');
                }
            },
            error: function(xhr, status, error) {
                console.error('Error fetching data from server:', error);
                $('#total').val('Error: Unable to fetch data');
            }
        });
    }
