...
<script>
$(document).ready(function () {
    var quantity = 1; // Initialize quantity to 1
    ...
    $('#button-addon2').on('click', function () {
        // Assuming this button is for updating the total
        // Set the value of the input field to the fetched price
        updateTotal();
    });
    ...
    // Click event for the Buy Now button
    $(document).on('click', '.buy-now-btn', function () {
        var menuID = $(this).data('id');
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function (data) {
                // Set the fetched price to the 'total' input field
                $('#total').val(data.Price);

                // Reset quantity to 1 when a new item is selected
                quantity = 1;
                // Update the total with the new quantity
                updateTotal();
            },
            error: function (xhr, status, error) {
                console.error(xhr.responseText);
    ...
    function updateTotal() {
        // Get the fetched price from the 'total' input field
        var fetchedPrice = parseFloat($('#total').val());

        // Check if fetchedPrice is a valid number
        if (!isNaN(fetchedPrice)) {
            var total = fetchedPrice * quantity;
            $('#total').val(total.toFixed(2));
            $('#quantity').val(quantity);
        } else {
            console.error('Invalid fetched price');
        }
    }
});
