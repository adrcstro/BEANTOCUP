...
<script>
    ...
    <script>
    $(document).ready(function () {
        var fetchedPrice = 25.00; // Default value
        ...
        $(document).on('click', '.buy-now-btn', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    // Check if data.Price is a valid number
                    if (!isNaN(parseFloat(data.Price)) && isFinite(data.Price)) {
                        // Set the value of the input field to the fetched price
                        $('#total').val(data.Price);

                        // Set the fetchedPrice as the default value
                        fetchedPrice = parseFloat(data.Price);

                        // Call the function to update total with the new fetchedPrice
                        updateTotal();
                    } else {
                        console.error("Invalid data received for Price");
                    }
                },
