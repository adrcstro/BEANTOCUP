...
// Get the MenuID from the AJAX request
$menuID = $_POST['menuID'];

// Fetch data from the database based on the MenuID
$sql = "SELECT * FROM ordertbl WHERE MenuID = $menuID";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    ...
    // Handle the case where no data is found for the given MenuID
...
if ($loggedInCostumerID !== null && $loggedInUsername !== null) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<input type='text' name='fullName' value='{$userData['Name']}' class='form-control' readonly style='background-color: #fff;'>";
        echo "</div>";
        echo "<div class='mb-3'>";
        echo "<label id='email' for='email'><i class='fas fa-envelope'></i> Email</label>";
        echo "<input type='text' name='email' value='{$userData['Email']}' class='form-control' readonly style='background-color: #fff;'>";
        echo "</div>";
        echo "<div class='mb-3'>";
        echo "<label id='phone'  for='phone'><i class='fas fa-phone'></i> Phone</label>";
        echo "<input type='text' name='phone' value='{$userData['Phone']}' class='form-control' readonly style='background-color: #fff;'>";
        echo "</div>";
        echo "<div class='mb-3'>";
        echo "<label id='homeAddress' for='homeAddress'><i class='fas fa-home'></i> Home Address</label>";
        echo "<input type='text' name='homeAddress' value='{$userData['HomeAddress']}' class='form-control' readonly style='background-color: #fff;'>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
...
<div  style="width: 80%; margin: auto; display:block;  margin-bottom: 10px;">
    <label id="modpay" for="modpay"></label>
...
<script>
    function updatepaymed(rating) {
        ...
        <div class="input-group-append">
            ...
            <button id="ordernow" style="background-color: #E48F45; color: #fff;" data-dismiss="modal" data-toggle="modal"  data-target="#payment"   type="button" class="btn  btn-sm m-1">
