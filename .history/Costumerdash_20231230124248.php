...
echo '<tr>
        <!-- ... (table data) ... -->
        <td><button class="btn btn-danger" onclick="deleteOrder(' . $row["orderid"] . ',' . $row["Amount"] . ')">Delete</button></td>
      </tr>';
...
?>
 ...
 <script>
function deleteOrder(orderId, orderAmount) {
    if (confirm('Are you sure you want to delete this order?')) {
        // Add AJAX or form submission logic to delete the order in the backend
        // For simplicity, I'll assume you have a PHP script to handle deletion
        // and provide the order ID as a query parameter
        var url = 'delete_order.php?orderId=' + orderId;

        // Perform AJAX request or form submission to delete the order
        // ...

        // Update the total amount by subtracting the deleted order amount
        var totalAmountElement = document.getElementById('totalAmount');
        var currentTotal = parseFloat(totalAmountElement.innerText.replace('$', '').replace(',', ''));
        var newTotal = currentTotal - orderAmount;

        // Display the updated total amount
        totalAmountElement.innerText = 'Total Amount: $' + newTotal.toFixed(2);
    }
}
</script>
