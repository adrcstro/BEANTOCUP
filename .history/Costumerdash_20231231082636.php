...
if ($loggedInCostumerID !== null && $loggedInUsername !== null) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $sqlDelivered = "SELECT * FROM ordertbl WHERE CostumerName = '$desiredCustomerName' AND Status = 'Delivered' ORDER BY OrderID DESC";
        ...
        $sqlOtherStatus = "SELECT * FROM ordertbl WHERE CostumerName = '$desiredCustomerName' AND Status IN ('In Transit', 'On Process') ORDER BY OrderID DESC";
        $resultOtherStatus = $conn->query($sqlOtherStatus);
        
        if ($resultOtherStatus->num_rows > 0) {
            echo '<h2>Orders In Transit or On Process</h2>';
            echo '<table class="table table-hover table-nowrap">
                    <thead class="thead-light">
                        <tr>
                            <th>Order-ID</th>
            ...
            // Output data of each row
            while ($row = $resultOtherStatus->fetch_assoc()) {
                echo '<tr>
                        <td>' . $row["OrderID"] . '</td>
                        <td>' . $row["ItemsOrdered"] . '</td>
                        <td>' . $row["Size"] . '</td>
            ...
            echo '<h2>No Orders In Transit or On Process</h2>';
        }
