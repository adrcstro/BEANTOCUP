...
$loggedInCustomerID = isset($_GET['CustomerID']) ? $_GET['CustomerID'] : null;
$loggedInUsername = isset($_GET['username']) ? $_GET['username'] : null;

if ($loggedInCustomerID !== null && $loggedInUsername !== null) {
    $query = "SELECT Name, Email, Phone, HomeAddress FROM customers WHERE CustomerID = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("s", $loggedInCustomerID);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "SELECT OrderID FROM ordertbl WHERE CustomerName = ? AND Status != 'Delivered'";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $desiredCustomerName);
        $stmt->execute();
        $result = $stmt->get_result();

        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo '<option value="' . htmlspecialchars($row["OrderID"]) . '">' . htmlspecialchars($row["OrderID"]) . '</option>';
            }
        } else {
            echo "No undelivered orders found.";
        }
    } else {
        echo "Error fetching user details.";
    }
