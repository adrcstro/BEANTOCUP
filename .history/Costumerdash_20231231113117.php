    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Check if the order status is not one of the specified values
                if ($status !== "Delivered" && $status !== "Intransit" && $status !== "Order Process") {
                    echo '<tr>
                            <td>' . $row["OrderID"] . '</td>
...
echo '</tbody></table>';
        ...
        If the Status stated in you Order is Blank/Empty it is Elegible for Update and Cancelation of Order<br>
        and if the order status has an input like On Process,Intransit and Delivered it is not capable in <br>
