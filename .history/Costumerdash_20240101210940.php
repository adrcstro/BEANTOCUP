...
if ($loggedInCostumerID !== null && $loggedInUsername !== null) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            // Display input fields for Items Ordered, Size, Quantity, and Amount
            echo '<form method="post" action="process_input.php">
                    <label for="itemsOrdered">Items Ordered:</label>
                    <input type="text" name="itemsOrdered" id="itemsOrdered" value="' . formatItemsOrderedForForm($result) . '" required><br>
                    
                    <label for="size">Size:</label>
                    <input type="text" name="size" id="size" value="' . $row["Size"] . '" required><br>
                    
                    <label for="quantity">Quantity:</label>
                    <input type="text" name="quantity" id="quantity" value="' . $row["QTY"] . '" required><br>
                    
                    <label for="amount">Amount:</label>
                    <input type="text" name="amount" id="amount" value="' . $row["Amount"] . '" required><br>
                    
                    <input type="submit" value="Submit">
                </form>';
            
            // Display the total amount outside the form with padding and centering
            echo '<div style="padding: 20px; text-align: center;">Total Amount: $' . $totalAmount . '</div>';
        } else {
            echo "0 results";
        }
...
function formatItemsOrdered($itemsOrdered) {
...
function formatItemsOrderedForForm($result) {
    $formattedItems = [];

    while ($row = $result->fetch_assoc()) {
        $formattedItems[] = formatItemsOrdered($row["ItemsOrdered"]);
    }
