    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Display a single row with 4 input fields or 1 input field if there are 2 or more values
                $inputField = count([$row["ItemsOrdered"], $row["Size"], $row["QTY"], $row["Amount"]]) > 1
                    ? '<td><input type="text" name="rowData[]" value="' . $dataToDisplay . '"></td>'
                    : '<td><input type="text" name="itemsOrdered[]" value="' . $row["ItemsOrdered"] . '"></td>
                       <td><input type="text" name="size[]" value="' . $row["Size"] . '"></td>
                       <td><input type="text" name="quantity[]" value="' . $row["QTY"] . '"></td>
                       <td><input type="text" name="amount[]" value="' . $row["Amount"] . '"></td>';
                
                echo '<tr>' . $inputField . '</tr>';
