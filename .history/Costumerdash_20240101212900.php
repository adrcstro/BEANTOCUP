...
// Display a single row with 4 input fields
function formatData($data) {
    // Check if the data contains a comma (,) and format accordingly
    return (strpos($data, ',') !== false) ? "($data)" : $data;
}

// Display a single row with 4 input fields and JavaScript
echo '<tr>
        <td><input type="text" name="itemsOrdered[]" value="' . formatData($row["ItemsOrdered"]) . '"></td>
        <td><input type="text" name="size[]" value="' . formatData($row["Size"]) . '"></td>
        <td><input type="text" name="quantity[]" value="' . formatData($row["QTY"]) . '"></td>
        <td><input type="text" name="amount[]" value="' . $row["Amount"] . '" id="totalAmountInput"></td>
    </tr>';

// JavaScript code to update the last input field with the total amount
echo '<script>
