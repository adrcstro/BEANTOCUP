...
// Additional input fields for filtering
$itemsOrderedFilter = isset($_POST['itemsOrderedFilter']) ? $_POST['itemsOrderedFilter'] : null;
$sizeFilter = isset($_POST['sizeFilter']) ? $_POST['sizeFilter'] : null;
$qtyFilter = isset($_POST['qtyFilter']) ? $_POST['qtyFilter'] : null;
...
if ($loggedInCostumerID !== null && $loggedInUsername !== null) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo '<form method="post" action="">
                <label for="itemsOrderedFilter">Items Ordered:</label>
                <input type="text" name="itemsOrderedFilter" id="itemsOrderedFilter" value="'.$itemsOrderedFilter.'">
                <label for="sizeFilter">Size:</label>
                <input type="text" name="sizeFilter" id="sizeFilter" value="'.$sizeFilter.'">
                <label for="qtyFilter">QTY:</label>
                <input type="text" name="qtyFilter" id="qtyFilter" value="'.$qtyFilter.'">
                <input type="submit" value="Filter">
              </form>';

        // Fetch data from the addtocarttbl table for a specific CostumerName and applied filters
        $desiredCustomerName = $userData['Name'];
        $sql = "SELECT * FROM addtocarttbl WHERE CostumerName = '$desiredCustomerName'";

        // Apply filters if provided
        if ($itemsOrderedFilter !== null) {
            $sql .= " AND ItemsOrdered = '$itemsOrderedFilter'";
        }
        if ($sizeFilter !== null) {
            $sql .= " AND Size = '$sizeFilter'";
        }
        if ($qtyFilter !== null) {
            $sql .= " AND QTY = '$qtyFilter'";
        }
