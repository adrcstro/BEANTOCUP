...
<script>
    $(document).ready(function () {
        ...
        function updateTextVisibility(boxIds) {
            ...
            for (var i = 0; i < boxIds.length; i++) {
                var backgroundColor = $('#' + boxIds[i]).css('background-color');
                if (backgroundColor !== 'rgba(0, 0, 0, 0)' && backgroundColor !== 'transparent' && backgroundColor !== 'none') {
                    // Change text color to white only when a background color is applied
                    $('#' + boxIds[i] + ' .status').css('color', '#fff');
                } else {
                    // Keep the original text color when no background color is applied
                    $('#' + boxIds[i] + ' .status').css('color', '');
                }
            }
        }
