...
<div class="d-flex flex-column flex-lg-row h-lg-full bg-surface-secondary">
    ...
    <nav class="navbar show navbar-vertical h-lg-screen navbar-expand-lg px-0 py-3 navbar-light bg-white border-bottom border-bottom-lg-0 border-end-lg" id="navbarVertical">
        <div class="container-fluid">
            ...
            <!-- Brand -->
            <a id="dashlogo" class="navbar-brand py-lg-2 mb-lg-5 px-lg-6 me-0" href="#">
...
<script>
    $(document).ready(function () {
        $('.status-button').click(function () {
            ...
            // Check if the status is empty or undefined
            if (!status) {
                setProgressBarWidth(progressBar, 0);
                showCheckIcons(['Ordered']);
                updateTextVisibility(['inprocess']);
                removeBackgroundColorFromBox(['inprocess']);
                return;
            }
            ...
            switch (status) {
                case 'Order Process':
                    ...
                    showCheckIcons(['Ordered', 'On Process']);
                    break;
                case 'Intransit':
                    setProgressBarWidth(progressBar, 63);
                    changeBoxColors(['inprocess', 'intransit']);
                    showCheckIcons(['Ordered', 'On Process', 'In Transit']);
                    break;
                case 'Delivered':
                    setProgressBarWidth(progressBar, 100);
                    changeBoxColors(['inprocess', 'intransit', 'delivered']);
                    showCheckIcons(['Ordered', 'On Process', 'In Transit', 'Delivered']);
                    break;
                default:
                    setProgressBarWidth(progressBar, 10);
                    // Reset box colors and hide all check icons if status is not recognized
                    resetBoxColors();
                    hideAllCheckIcons();
            }
        });
        ...
        function showCheckIcons(statuses) {
            // Hide all check icons
            hideAllCheckIcons();

            // Show check icon for specified statuses
            for (var i = 0; i < statuses.length; i++) {
                $('p:contains(' + statuses[i] + ')').prev('.completed-circle').show();
            }
        }

        function hideAllCheckIcons() {
            // Hide all check icons
            $('.completed-circle').hide();
        }

        function removeBackgroundColorFromBox(boxIds) {
            // Remove background color from specified boxes
            for (var i = 0; i < boxIds.length; i++) {
                $('#' + boxIds[i]).css('background-color', '');
            }
        }

        function updateTextVisibility(boxIds) {
            // Update text visibility for specified boxes
            for (var i = 0; i < boxIds.length; i++) {
                $('#' + boxIds[i] + ' .status').css('color', '#000'); // Change color to your desired value
            }
        }
        ...
        function changeBoxColors(boxIds) {
            ...
            for (var i = 0; i < boxIds.length; i++) {
                ...
                $('#' + boxId).css({
                    'background-color': '#994D1C',
                    'color': '#fff' // Set text color to white when background color changes
                });

                // Update text color to gray if the background color is empty or not set
                if (!$('#' + boxId).css('background-color') || $('#' + boxId).css('background-color') === 'rgba(0, 0, 0, 0)') {
                    $('#' + boxId + ' .status').css('color', '#808080'); // Set text color to gray
                }
            }
        }
    ...
    for (var i = 0; i < boxIds.length; i++) {
        var boxId = boxIds[i];
        var darkBrownColor = '#994D1C';

        $('#' + boxId).css('background-color', darkBrownColor);

        // Set text color to white only when the background color changes to dark brown
        if ($('#' + boxId).css('background-color') === darkBrownColor) {
            $('#' + boxId + ' .status').css('color', '#fff'); // Set text color to white
        } else {
            // Update text color to gray if the background color is empty or not set
            $('#' + boxId + ' .status').css('color', '#808080'); // Set text color to gray
        }
...
<button style="background-color: #E48F45; color: #fff;" type="button" class="btn btn btn-sm m-1" data-toggle="modal"  data-target="#Receipt">
<i class="bi bi-card-heading"></i> View Receipt</button>


<button style="background-color: #E48F45; color: #fff;" type="button" class="btn btn btn-sm m-1" data-toggle="modal"  data-target="#onlinescan">
<i class="bi bi-qr-code"></i> Pay Online</button>


 <button style="background-color: #E48F45; color: #fff;" type="button" class="btn btn btn-sm m-1" id="refreshButton12">
        <i class="bi bi-card-heading"></i> Refresh
    </button>
<script>
...
<div class="modal fade" id="cancelorder">
    <div class="modal-dialog">
        ...
        <div style="background-color: #E48F45;" class="modal-header">
            ...
            <div class="modal-body">
                <form id="Costumercancelorder" action="trackdelete.php" method="post">
                    <div class="form-group">
...
<script>
   $(document).ready(function() {
    $('#Costumercancelorder').submit(function(e) {
        e.preventDefault();
        var formData = $(this).serialize();
        $.ajax({
            type: 'POST',
...
<div class="modal fade" id="Receipt">
    <div class="modal-dialog" id="sizereciept">
        <div class="modal-content">
        <div style="background-color: #E48F45;" class="modal-header">
                    <h4 style="color: #fff;" class="modal-title">View Order Receipt</h4>
                    <button style="background-color: #E48F45; color: #fff;" type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
            <div class="modal-body">
            <form id="reciptorder" action="Generatereciept.php" method="post" enctype="multipart/form-data">
    <div class="form-group">
        <label for="Cosreceipt">Print Selected Receipt</label>
        <select name="Cosreceipt" id="Cosreceipt" class="form-control" required>
            <option value="" disabled selected>Select Order ID</option>
    ...
    if ($loggedInCostumerID !== null && $loggedInUsername !== null) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            // Fetch data from the ordertbl table for a specific CostumerName
            $desiredCustomerName = $userData['Name'];
            $sql = "SELECT OrderID FROM ordertbl WHERE CostumerName = '$desiredCustomerName'";
            $result = $conn->query($sql);
    ...
    ?>
           ...
           <div id="OrderResult"></div>
                    ...
                    <div class="modal-footer">
        <button style="background-color: #E48F45; color: #fff;" type="button" class="btn btn" data-dismiss="modal">Close</button>
        <button style="background-color: #E48F45; color: #fff;" id="printreceipt" type="submit" class="btn btn"> <i class="fas fa-print"></i> Print</button>
    </div>
</form>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const selectInput = document.getElementById("Cosreceipt");
        const searchResults = document.getElementById("OrderResult");

        selectInput.addEventListener("change", function () {
            const selectedValue = selectInput.value;
            if (selectedValue.length > 0) {
                fetch(`CostumerOrderdetails.php?search=${selectedValue}`)
...
<script>
    document.getElementById('printreceipt').addEventListener('click', function () {
        ...
        document.getElementById('reciptorder').target = "_blank";
        // Submit the form
        document.getElementById('reciptorder').submit();
        // Reset the form target to null after submission
        document.getElementById('reciptorder').target = null;
    });
</script>
...
<div class="modal fade" id="onlinescan">
    <div id="scanqr" class="modal-dialog">
        <div class="modal-content">
        <div style="background-color: #E48F45;" class="modal-header">
                    <h4 style="color: #fff;" class="modal-title">We Accept Maya$Gcash Payment</h4>
                    <button style="background-color: #E48F45; color: #fff;" type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
            <div class="modal-body">
  ...
  <div class="row justify-content-center">
    <div class="col-md-6 mb-4 d-flex">
      <div class="card shadow uniwqe flex-column h-100">
        <img src="../Images/Gcash.jpg" class="card-img-top" alt="Card Image">
        <div class="card-body d-flex flex-column">
          <div class="text-center">
            <h5 class="card-title mb-1">G-Cash</h5>
            <p class="card-text mt-2">We accept G-cash Payment just scan the Qr-Code and Give it to the rider as a Prof</p>
          </div>
          <div class="shortcutbtn mt-2 d-flex justify-content-center align-items-center">
            <a href="https://m.gcash.com/gcash-login-web/index.html#/" class="btn d-inline-flex btn-sm btn-neutral border-base mx-1" target="_blank">
              <img src="../Images/gcash.jpeg" alt="GCash Logo" class="me-2" width="20" height="20">
              <span>Select</span>
            </a>
          </div>
        </div>
    ...
    <div class="col-md-6 mb-4 d-flex">
      <div class="card shadow uniwqe flex-column h-100">
        <img src="../Images/maya.jpg" class="card-img-top" alt="Card Image">
        <div class="card-body d-flex flex-column">
          <div class="text-center">
            <h5 class="card-title mt-3 ">Maya</h5>
            <p class="card-text mt-2 mt-auto text-center">We accept Maya Payment just scan the Qr-Code and Give it to the rider as a Prof</p>
          </div>
          <div class="shortcutbtn mt-2 d-flex justify-content-center align-items-center">
            <a href="https://www.maya.ph/app/registration?invite=426250Y9Q6P0" class="btn d-inline-flex btn-sm btn-neutral border-base mx-1" target="_blank">
              <img src="../Images/maya-logo.png" alt="Maya Logo" class="me-2" width="20" height="20">
              <span>Select</span>
            </a>
          </div>
        </div>
...
<div class="menu-btns">
    <button type="button" class="menu-btn" data-toggle="modal"  data-target="#hotcoffees" id="featured"><i class="material-icons">local_cafe</i>Hot Coffee's</button>
    <button type="button" class="menu-btn" data-toggle="modal"  data-target="#Pastries" id="today-special"><i class="material-icons">cake</i>Pastries</button>
    <button type="button" class="menu-btn" data-toggle="modal"  data-target="#IceCoffees" id="new-arrival"><i class="material-icons">ac_unit</i>Ice Coffee's</button>
    <button type="button" class="menu-btn" data-toggle="modal"  data-target="#BlendedCoffees" id="Blended-Coffee"><i class="material-icons">kitchen</i>Blended Coffee's</button>
    <button type="button" class="menu-btn" data-toggle="modal"  data-target="#hotteas" id="Hot-tea"><i class="material-icons">whatshot</i>Hot Tea's</button>
</div>
...
$query = "SELECT MenuID, Name, Price, Body, Image FROM menu WHERE MenuID IN (20, 21,22, 23, 24, 25) AND (MenuID LIKE '%$searchTerm%' OR Name LIKE '%$searchTerm%' OR Price LIKE '%$searchTerm%' OR Body LIKE '%$searchTerm%' OR Image LIKE '%$searchTerm%')";
