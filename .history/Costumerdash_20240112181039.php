 ...
 <script>
   $(document).ready(function () {
    $('.status-button').click(function () {
        ...
        // Update progress bar and check icons based on status
        var progressBar = $('.progress-bar');
        switch (status) {
            case 'Order Process':
                setProgressBarAndCheckIcons(progressBar, 38, ['none', 'inprocess']);
                break;
            case 'Intransit':
                setProgressBarAndCheckIcons(progressBar, 63, ['none', 'inprocess', 'intransit']);
                break;
            case 'Delivered':
                setProgressBarAndCheckIcons(progressBar, 100, ['none', 'inprocess', 'intransit', 'delivered']);
                break;
            default:
                setProgressBarAndCheckIcons(progressBar, 10, ['none']);
                // Reset box colors to default if status is not recognized
                resetBoxColors();
        }
    });

    function setProgressBarAndCheckIcons(progressBar, width, completedStages) {
        setProgressBarWidth(progressBar, width);
        changeBoxColors(completedStages);
        showCheckIcons(completedStages);
    }

    function showCheckIcons(completedStages) {
        // Hide all check icons
        $('.completed-circle i').hide();

        // Show check icons for completed stages
        for (var i = 0; i < completedStages.length; i++) {
            $('#' + completedStages[i] + ' .completed-circle i').show();
        }
    }
