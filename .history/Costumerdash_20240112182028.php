...
?>
 ...
 $(document).ready(function () {
    $('.status-button').click(function () {
        ...
        switch (status) {
            case 'Ordered':
                ...
                updateCheckIcons(['Ordered']);
                break;
            case 'Order Process':
                setProgressBarWidth(progressBar, 50);
                updateCheckIcons(['Ordered', 'Order Process']);
                break;
            case 'Intransit':
                setProgressBarWidth(progressBar, 75);
                updateCheckIcons(['Ordered', 'Order Process', 'In Transit']);
                break;
            case 'Delivered':
                setProgressBarWidth(progressBar, 100);
                updateCheckIcons(['Ordered', 'Order Process', 'In Transit', 'Delivered']);
                break;
            default:
    ...
    function updateCheckIcons(statuses) {
        // Hide all check icons
        resetCheckIcons();

        // Show check icon for specified statuses
        for (var i = 0; i < statuses.length; i++) {
            $('div.completed-circle[data-status="' + statuses[i] + '"]').show();
        }
    }

    function resetCheckIcons() {
        // Hide all check icons
        $('div.completed-circle').hide();
    }
