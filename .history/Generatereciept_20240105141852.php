    ...
    if (isset($_POST["Cosreceipt"])) {
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $pdf->Cell(0, 10, "Costumer Name: " . $row['CostumerName'], 0, 1);
                $pdf->Cell(0, 10, "Date Placed: " . $row['DatePlaced'], 0, 1);
                $pdf->Cell(0, 10, "Shipping Address: " . $row['ShippingAddress'], 0, 1);
                $pdf->Cell(0, 10, "Contact Number: " . $row['ContactNumber'], 0, 1);
                $pdf->Cell(0, 10, "Email Address: " . $row['EmailAddress'], 0, 1);
                $pdf->Cell(0, 10, "Payment Method: " . $row['PaymentMethod'], 0, 1);

                // Output items in a table
                $pdf->SetFillColor(200, 220, 255); // Set the background color for the first row
                $pdf->Cell(40, 10, "Items Ordered", 1, 0, 'C', true);
                $pdf->Cell(40, 10, "Size", 1, 0, 'C', true);
                ...
                // Output total amount
                $pdf->Cell(0, 10, "Total Amount: " . $row['Amount'], 0, 1);
            }
