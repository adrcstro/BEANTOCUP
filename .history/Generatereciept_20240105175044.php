...
// Display order details as a table with 5 columns and borders
$pdf->SetFont('times', 'B', 12);
...
for ($i = 0; $i < count($labels); $i++) {
    ...
    // Add corresponding data in the second column
    $pdf->Cell($cellWidth, 10, $data[$i], 1, 0, 'L');

    // Add extra space after every 2 columns
    if (($i + 1) % 2 === 0) {
        $pdf->Cell(10, 10, '', 0, 0, 'L');
    }

    // Move to the next line after every 5 columns
    if (($i + 1) % 5 === 0) {
        $pdf->Ln();
    }
}
