...
// Display order details as a table with borders


// Display order details as a table with 2 rows and borders
// Display order details as a table with 5 columns and titles
$pdf->SetFont('times', 'B', 12);
...
$columnTitles = ["Column 1 Title", "Column 2 Title", "Column 3 Title", "Column 4 Title", "Column 5 Title"];

// Labels for the order details
$labels = ["Payment Method:", "Items Ordered:", "Size:", "Quantity:", "Amount:"];
...
// Loop to create titles for each column
for ($i = 0; $i < count($columnTitles); $i++) {
    $pdf->Cell($cellWidth, 10, $columnTitles[$i], 1, 0, 'C');
}

// Move to the next line after the titles
$pdf->Ln();

// Set font for the data
$pdf->SetFont('times', '', 12);

// Loop to create rows of data
for ($i = 0; $i < count($labels); $i++) {
    // Add label in the first column
    $pdf->Cell($cellWidth, 10, $labels[$i], 1, 0, 'L');

    // Add corresponding data in the next columns
    $pdf->Cell($cellWidth, 10, $data[$i], 1, 0, 'L');
}
