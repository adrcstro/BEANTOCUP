...
$orderDetails = "Payment Method: {$row['PaymentMethod']}\nItems Ordered: {$row['ItemsOrdered']}\nSize: {$row['Size']}\nQuantity: {$row['QTY']}\nAmount: {$row['Amount']}";

// Create an array of lines from the order details
$orderDetailsLines = explode("\n", $orderDetails);

// Set cell height
$cellHeight = 10;

// Loop through each line and add it as a MultiCell
foreach ($orderDetailsLines as $line) {
    $pdf->MultiCell(0, $cellHeight, $line, 0, 'L');
}
