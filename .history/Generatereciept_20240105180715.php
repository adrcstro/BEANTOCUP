...
for ($i = 0; $i < count($headers); $i++) {
    ...
    // For 'Items Ordered' column, use MultiCell to avoid text overlap
    if ($i == 0) {
        $pdf->MultiCell($columnWidths[$i], $cellHeight, $headers[$i], '1', 'L');
    } else {
...
for ($i = 0; $i < count($data); $i++) {
    ...
    // For 'Items Ordered' column, use MultiCell to avoid text overlap
    if ($i == 0) {
        $pdf->MultiCell($columnWidths[$i], $cellHeight, $data[$i], '1', 'L');
    } else {
