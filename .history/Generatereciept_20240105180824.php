...
// Define the column widths// Define the column widths
$columnWidths = array(70, 40, 30, 50, 60);
...
for ($i = 0; $i < count($headers); $i++) {
    ...
    // Calculate the dynamic width for 'Items Ordered' column
    $dynamicWidth = $columnWidths[$i];
    if ($i == 0) {
        $dynamicWidth = min($dynamicWidth, $pdf->GetStringWidth($row['ItemsOrdered']) + 6); // Add some padding
    }

    // Use MultiCell for 'Items Ordered' to avoid text overlap
    $pdf->MultiCell($dynamicWidth, $cellHeight, $headers[$i], '1', 'L');
}
...
for ($i = 0; $i < count($data); $i++) {
    ...
    // Use MultiCell for 'Items Ordered' to avoid text overlap
    $pdf->MultiCell($dynamicWidth, $cellHeight, $data[$i], '1', 'L');
}
