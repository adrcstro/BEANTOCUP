...
for ($i = 0; $i < count($headers); $i++) {
    ...
    // Determine the number of lines needed for the current data item
    $numLines = ceil($pdf->getStringWidth($data[$i]) / $columnWidths[$i]);

    // Add cell with specified width, content, and border
    $pdf->MultiCell($columnWidths[$i], $cellHeight * $numLines, $headers[$i], '1', 'L');
}
...
for ($i = 0; $i < count($data); $i++) {
    ...
    // Add cell with specified width, content, and border
    $pdf->MultiCell($columnWidths[$i], $cellHeight * $numLines, $data[$i], '1', 'L');
}
