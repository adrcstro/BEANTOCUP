        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                for ($i = 0; $i < count($headers); $i++) {
                    // Set cell height based on available width
                    $cellHeight = $pdf->GetStringHeight($headers[$i]) + 2;
                ...
                for ($i = 0; $i < count($data); $i++) {
                    // Set cell height based on available width
                    $cellHeight = $pdf->GetStringHeight($data[$i]) + 2;
            ...
            $pdf->Output('OrderReceipt.pdf', 'D'); // 'D' prompts the browser to download the PDF
        } else {
