...
while ($row = $result->fetch_assoc()) {
    ...
    $maxWidth = $pdf->getPageWidth() - 20; // Adjust the margin as needed
    $totalColumns = count($headers);
    $dynamicColumnWidth = $maxWidth / $totalColumns;

    // Set font and cell height
    $fontMaxSize = 12;
    $fontSize = min($fontMaxSize, ($maxWidth / $dynamicColumnWidth) * 2);
    $pdf->SetFont('times', '', $fontSize);
    $cellHeight = 10;

    // Loop through headers and data to create the table with dynamic column widths
    for ($i = 0; $i < count($headers); $i++) {
        $pdf->Cell($dynamicColumnWidth, $cellHeight, $headers[$i], '1', 0, 'L');
    }

    // Move to the next line to start the second row
    $pdf->Ln();

    // Loop through data for the second row with dynamic column widths
    for ($i = 0; $i < count($data); $i++) {
        $pdf->Cell($dynamicColumnWidth, $cellHeight, $data[$i], '1', 0, 'L');
    }
