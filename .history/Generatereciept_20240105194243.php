...
for ($i = 0; $i < count($data); $i++) {
    ...
    // Check if the text height exceeds the cell height
    if ($pdf->getStringHeight($columnWidths[$i], $data[$i]) > $cellHeight) {
        // If yes, split the text into multiple lines
        $lines = $pdf->splitText($columnWidths[$i], $data[$i]);
        foreach ($lines as $line) {
            $pdf->Cell($columnWidths[$i], $cellHeight, $line, '1', 0, 'C');
            $pdf->Ln();
        }
    } else {
        // If no, add the text to the cell normally
