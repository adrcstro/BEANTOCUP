...
$pdf->MultiCell(implode('', array_map(function($i) use ($columnWidths, $cellHeight) {
    return $pdf->SetWidth($columnWidths[$i])->SetHeight($cellHeight)->SetValue($data[$i])->SetAlign('C')->Render();
}, range(0, count($data) - 1))), '1', 'C');
