...
// Loop through data for the single row
// Loop through data for the single row
for ($i = 0; $i < count($data); $i++) {
    // Set maximum character length for each cell
    $maxCellLength = 20; // You can adjust this value as needed

    // Trim the text if it exceeds the maximum length
    $trimmedText = (strlen($data[$i]) > $maxCellLength) ? substr($data[$i], 0, $maxCellLength) . '...' : $data[$i];

    // Add cell with specified width, content, and border
    $pdf->Cell($columnWidths[$i], $cellHeight, $trimmedText, '1', 0, 'C');

    // If text was trimmed, add an extra row below with the full text
    if (strlen($data[$i]) > $maxCellLength) {
        $pdf->Ln();
