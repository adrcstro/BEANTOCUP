...
for ($i = 0; $i < count($data); $i++) {
    // Determine the maximum character length that can fit in the cell
    $maxCharLength = floor($columnWidths[$i] / $pdf->GetStringWidth("W"));
    $cellHeight = 100;
    // Check if the data exceeds the maximum length
    if (strlen($data[$i]) > $maxCharLength) {
        // Calculate the reduced font size to fit the content
        $reducedFontSize = 12;

        while ($pdf->GetStringWidth($data[$i]) > $columnWidths[$i] - 2 && $reducedFontSize > 6) {
            $reducedFontSize--;
            $pdf->SetFontSize($reducedFontSize);
        }

        // Add multi-cell with specified width, height, content, and border
...
// Reset font size to the original after the table
$pdf->SetFontSize(12);
$pdf->Ln();
