...
// Define the calculateFontSize function
function calculateFontSize($pdf, $text, $width, $maxFontSize)
{
    $fontSize = $maxFontSize;

    // Adjust font size if text length exceeds cell width
    while ($pdf->GetStringWidth($text) > $width - 2 && $fontSize > 2) {
        $fontSize--;
...
for ($i = 0; $i < count($headers); $i++) {
    // Set cell height
    $cellHeight = 20; // Adjust the cell height as needed

    // Set font size
    $fontSize = 12;
    $pdf->SetFont('times', '', $fontSize);

    // Add cell with specified width, height, and border
    $pdf->Cell($columnWidths[$i], $cellHeight, '', '1');

    // Set cell content
    $pdf->SetXY($pdf->GetX(), $pdf->GetY() - $cellHeight / 2);
    $pdf->MultiCell($columnWidths[$i], 10, $headers[$i]);
...
for ($i = 0; $i < count($data); $i++) {
    ...
    // Set cell content
    $pdf->SetXY($pdf->GetX(), $pdf->GetY() - $cellHeight / 2);
    $pdf->MultiCell($columnWidths[$i], 10, $data[$i]);
