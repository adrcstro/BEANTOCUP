...
for ($i = 0; $i < count($data); $i++) {
    ...
    $characters = str_split($data[$i]);

    // Loop through characters
    foreach ($characters as $char) {
        // Add cell with specified width, height, and border for data
        $pdf->Cell($columnWidths[$i], $cellHeight, $char, '1', 0, 'C');
        // Move to the next line after each character
        $pdf->SetY($pdf->GetY() + $cellHeight);
    }

    // Move to the next line after the column is complete
    $pdf->SetY($pdf->GetY() - $cellHeight * count($characters));
    $pdf->SetX($pdf->GetX() + $columnWidths[$i]); // Move to the next column
}
