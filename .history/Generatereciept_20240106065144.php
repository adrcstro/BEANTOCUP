...
for ($i = 0; $i < max(count($headers), count($data)); $i++) {
    // Set font size
    $fontSize = 12;
    $pdf->SetFont('times', '', $fontSize);

    // Add cell with specified width, height, and border for header
    if (isset($headers[$i])) {
        $pdf->MultiCell($columnWidths[$i], $cellHeight, $headers[$i], '1', 'C');
    } else {
        $pdf->Cell($columnWidths[$i], $cellHeight, '', '1');
    }

    // Add cell with specified width, height, and border for data
    if (isset($data[$i])) {
        $pdf->MultiCell($columnWidths[$i], $cellHeight, $data[$i], '1', 'C');
    } else {
        $pdf->Cell($columnWidths[$i], $cellHeight, '', '1');
    }
}
