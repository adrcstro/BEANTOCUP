...
for ($i = 0; $i < count($headers); $i++) {
    ...
    // Add cell with specified width, content, and border for the first column
    $pdf->Cell($columnWidths[0], $cellHeight, $headers[$i], '1', 0, 'C');

    // Add multi-cell for the second column with adjusted content
    $pdf->MultiCell($columnWidths[1], $cellHeight, $i == 1 ? 'Modified Text: ' . $data[$i] : $data[$i], '1', 'C');
    
    // Add cell for the third column
    $pdf->Cell($columnWidths[2], $cellHeight, $data[$i], '1', 0, 'C');
}
