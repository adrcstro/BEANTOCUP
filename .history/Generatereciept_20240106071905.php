...
// Loop through headers and data to create the table
// Calculate the total width of the three columns
...
for ($i = 0; $i < count($data); $i++) {
    ...
    $dataParts = explode(',', $data[$i]);

    // Loop through data parts
    for ($j = 0; $j < count($dataParts) && $j < count($columnWidths); $j++) {
        // Set cell height
        $cellHeight = 10;

        // Add cell with specified width, content, and border
        $pdf->Cell($columnWidths[$j], $cellHeight, $dataParts[$j], '1', 0, 'C');
    }
