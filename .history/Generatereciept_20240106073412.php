...
// Loop for data rows (limit to 2 additional rows)
for ($i = 0; $i < min(2, count($data)); $i++) {
    // Check if MultiCell was successful
    if (!$pdf->MultiCell($columnWidths[$i], $cellHeight, $data[$i], '1', 'C')) {
        // Handle the error, or exit the loop to prevent further issues
        break;
    }
