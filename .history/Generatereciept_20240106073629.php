...
// Concatenate data elements into a single string
for ($i = 0; $i < count($data); $i++) {
    $rowData .= $data[$i];

    // Add a space between data elements (adjust as needed)
    $rowData .= ' ';

    // Optionally, add a line break between data elements (adjust as needed)
    // $rowData .= "\n";
}

// Add a MultiCell for the entire row
$pdf->MultiCell(array_sum($columnWidths), $cellHeight, $rowData, '1', 'C');

// Move to the next line after the row (adjust as needed)
$pdf->Ln();
