...
// Adjust column widths based on the length of the header text
foreach ($headers as $i => $header) {
    $columnWidths[$i] = max($columnWidths[$i], $pdf->GetStringWidth($header) + 6); // Adding a small padding
}
...
for ($i = 0; $i < count($data); $i++) {
    for ($j = 0; $j < count($data[$i]); $j++) {
        // Adjust column widths based on the length of the data text
        $columnWidths[$j] = max($columnWidths[$j], $pdf->GetStringWidth($data[$i][$j]) + 6); // Adding a small padding

        // Add cell with specified width, content, and border
        $pdf->Cell($columnWidths[$j], $cellHeight, $data[$i][$j], '1', 0, 'C');
    }
