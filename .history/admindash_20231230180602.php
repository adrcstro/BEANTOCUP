...
<div class="modal fade" id="Orderprocess">
        <div class="modal-dialog" id="orderprocessmodal">
            <div class="modal-content">
                ...
                <div class="modal-body">
                <form id="updateForm"  method="post">
                <label for="SelectOrderStatus">Select Order ID</label>
                        <select name="SelectOrderStatus" id="SelectOrderStatus" class="form-control" required>
                            <option value="" disabled selected>Select an option</option>
                            <?php
...
<div style="width: 80%; margin: auto; display:block;  margin-bottom: 10px;">
    <label for="orderstat"></label>
    <textarea class="form-control mx-auto" name="orderstat" id="orderstat" style="width: 100%;"></textarea>
</div>

<script>
    function updateRating(rating) {
        document.getElementById("orderstat").value = rating + "";
...
<div class="modal-footer">
  ...
  <script>
    ...
    function updateStatus() {
        var orderId = document.getElementById("SelectOrderStatus").value;
        var newStatus = document.getElementById("orderstat").value;
            ...
            if (this.readyState == 4) {
                ...
                if (this.status == 200) {
                    ...
                    if (this.responseText.trim() === 'success') {
                        ...
                        // Log an error message if the response is not 'success'
                        console.error("Status update failed. Server response:", this.responseText);
                    }
                } else {
        ...
        // Send the request to update.php with parameters
        xhttp.open("POST", "update.php", true);
        xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhttp.send("orderId=" + orderId + "&newStatus=" + newStatus);
    }

    // Attach a click event to the "Save Status" button
