...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['PlacefullName'], $_POST['PlacehomeAddress'], $_POST['Placephone'], $_POST['Placeemail'], $_POST['placeitemsordered'], $_POST['placesize'], $_POST['placeqty'], $_POST['placetotalamount'])){
    // Collect form data for the insertion
    ...
    if ($placeFullName !== null) {
        // Prepare and bind for the insertion
        $stmt = $conn->prepare("INSERT INTO ordertbl (CostumerName, ShippingAddress, ContactNumber, EmailAddress,  ItemsOrdered, Size, QTY, Amount) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?)");

        // Use "sssssssssd" as the bind_param string
        $stmt->bind_param("sssssssd", $placeFullName,  $placeHomeAddress, $placePhone, $placeEmail, $placeProdname, $placeSize, $placeQuantity, $placeTotal);
