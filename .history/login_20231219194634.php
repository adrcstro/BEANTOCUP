...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($userType === 'customer') {
        // Define the table for customers
        $table = 'costumers';
    } elseif ($userType === 'admin') {
        // Define the table for admins
        $table = 'admintbl';
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Set a unique session name based on the username
        session_name("user_" . $username);
        session_start(); // Start a new session or resume the existing one

        // Assign customer ID and username to session variables
