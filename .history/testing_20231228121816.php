...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['fullName'], $_POST['dateplace'], $_POST['homeAddress'], $_POST['phone'], $_POST['email'], $_POST['modpay'], $_POST['prodname'], $_POST['size'], $_POST['quantity'], $_POST['total'])){
    ...
    if ($FullName !== null) {
        ...
        $stmt = $conn->prepare("INSERT INTO addtocarttbl (CostumerName, DatePlaced, ShippingAddress, ContactNumber, EmailAddress, PaymentMethod, ItemsOrdered, Size, QTY, Amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        // Use "sssssssssd" as the bind_param string
        $stmt->bind_param("ssssssssssd", $FullName, $Dateplace, $HomeAddress, $Phone, $Email, $Modpay, $Prodname, $Size, $Quantity, $Total);
