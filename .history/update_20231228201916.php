...
function  updateorders($conn) {
    ...
    if (!empty($newsid)) {
        $stmt = $conn->prepare("UPDATE ordertbl SET Name=?, Price=?, Body=? WHERE uporderID=?");
        $stmt->bind_param("sssssssssd", $upAddorcostumerName, $upAddorDateOrdered, $upAddorShippingAddress, $upAddorContactNumber, $upAddorEmailAddress , $upAddorPAymentmethod, $upAddorItemsOrdered, $upAddorSize,  $upAddorQuantity, $upAddoramount);
