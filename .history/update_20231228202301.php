...
function  updateorders($conn) {
    ...
    if (!empty($UporderID)) {
        $stmt = $conn->prepare("UPDATE ordertbl SET CostumerName=?, DatePlaced=?, ShippingAddress=? , ContactNumber=?,EmailAddress=? , PaymentMethod=?, ItemsOrdered=?, Size=? ,QTY=?,Amount=? WHERE uporderID=?");
        $stmt->bind_param("sssssssssd", $upAddorcostumerName, $upAddorDateOrdered, $upAddorShippingAddress, $upAddorContactNumber, $upAddorEmailAddress , $upAddorPAymentmethod, $upAddorItemsOrdered, $upAddorSize,  $upAddorQuantity, $upAddoramount ,$UporderID);
