...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['SelectOrderStatus'])) {
        updatestatus($conn);
    }
...
function updatestatus($conn) {
    ...
    if (!empty($Selectedstatus)) {
        // Assuming OrderID is an integer, change 's' to 'i' in bind_param
        $stmt = $conn->prepare("UPDATE ordertbl SET Status=? WHERE OrderID=?");
        
        // Bind one parameter as string and the other as integer
        $stmt->bind_param("si", $Selectedstatus, $Orderstat);
