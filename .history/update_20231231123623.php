...
function   updatetrackuporderIDorders($conn) {
    ...
    $TrackupAddorcostumerName = $_POST['trackupaddorcostumerName'];
    ...
    if (!empty($TrackorderID)) {
        $stmt = $conn->prepare("UPDATE ordertbl SET CostumerName=?, DatePlaced=?, ShippingAddress=? , ContactNumber=?,EmailAddress=? WHERE OrderID=?");
        $stmt->bind_param("ssssss", $TrackupAddorcostumerName, $TrackupAddorDateOrdered, $TrackupAddorShippingAddress, $TrackupAddorContactNumber, $TrackupAddorEmailAddress , $TrackorderID );
